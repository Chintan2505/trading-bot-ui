    ...
    <div className="flex-1 overflow-y-auto">
      ...
      <div className="flex items-center justify-between px-6 py-4 border-b border-terminal-border">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-[12px] text-gray-500">{positions.length} open position{positions.length !== 1 ? 's' : ''}</p>
          </div>
        </div>
      ...
      {showCloseAllConfirm && (
        <div className="mx-6 mt-4 p-4 rounded-xl border border-bear/30 bg-bear/5">
          ...
          <p className="text-[12px] text-gray-400 mb-3">This will close all {positions.length} open positions at market price.</p>
      ...
      {positions.length > 0 && (
        <div className="px-6 py-3 border-b border-terminal-border flex items-center gap-6">
          <div>
            <span className="text-[10px] uppercase tracking-wider text-gray-500 font-semibold">Total Value</span>
            <div className="text-sm font-mono font-bold text-white">${totalValue.toLocaleString('en-US', { minimumFractionDigits: 2 })}</div>
          </div>
          <div>
            <span className="text-[10px] uppercase tracking-wider text-gray-500 font-semibold">Unrealized P&L</span>
            <div className={`text-sm font-mono font-bold ${totalPL >= 0 ? 'text-bull' : 'text-bear'}`}>
      ...
      <div className="px-6 py-4">
        {loading ? (
          ...
          <div className="flex flex-col items-center justify-center py-20 text-gray-600">
            ...
            <p className="text-[12px] text-gray-700 mt-1">Your positions will appear here when you start trading</p>
          </div>
        ) : (
            ...
            <table className="w-full">
              <thead>
                <tr className="border-b border-terminal-border">
                  {['Symbol', 'Side', 'Qty', 'Avg Entry', 'Current', 'Market Value', 'P&L', 'P&L %', 'Action'].map(h => (
              ...
              <tbody className="divide-y divide-terminal-border">
                {positions.map((pos) => {
                  ...
                  const isUp = pl >= 0;
                  return (
                    <tr key={pos.symbol} className="hover:bg-terminal-hover/50 transition-colors">
                      ...
                      <td className="py-2.5 px-3 text-sm font-mono text-gray-300">{pos.qty}</td>
                      <td className="py-2.5 px-3 text-sm font-mono text-gray-300">${parseFloat(pos.avg_entry_price || 0).toFixed(2)}</td>
                      <td className="py-2.5 px-3 text-sm font-mono text-white">${parseFloat(pos.current_price || 0).toFixed(2)}</td>
                      <td className="py-2.5 px-3 text-sm font-mono text-gray-300">${parseFloat(pos.market_value || 0).toLocaleString('en-US', { minimumFractionDigits: 2 })}</td>
                      <td className={`py-2.5 px-3 text-sm font-mono font-medium ${isUp ? 'text-bull' : 'text-bear'}`}>
