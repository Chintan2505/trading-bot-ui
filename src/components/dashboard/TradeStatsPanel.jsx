...
const fmt = (v) =>
  typeof v === "number"
    ...
    : "$0.00";
const fmtPct = (v) =>
  typeof v === "number" ? `${v >= 0 ? "+" : ""}${v.toFixed(1)}%` : "0.0%";
const pnlColor = (v) =>
  typeof v === "number" && v > 0
    ? "text-bull"
    : typeof v === "number" && v < 0
...
const PERIODS = ["today",  "allTime"];
const PERIOD_LABELS = { today: "Today", allTime: "All Time" };
...
const StatCard = ({
  ...
  <div className="p-2.5 rounded-lg bg-terminal-bg/80 border border-terminal-border hover:border-terminal-border/80 transition-colors">
    ...
    <div
      className={`text-sm font-bold font-mono ${valueClass || "text-white"}`}
    >
      {value}
    </div>
    {subValue && (
...
export default function TradeStatsPanel({ stats }) {
  ...
  return (
    <div className="space-y-2.5">
      ...
      <div className={`p-3 rounded-xl border ${pnlBg(s.totalPnl)}`}>
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-2">
            {(s.totalPnl ?? 0) >= 0 ? (
              <div className="w-8 h-8 rounded-lg bg-bull/10 flex items-center justify-center">
                <TrendingUp className="w-4 h-4 text-bull" />
              </div>
            ) : (
              <div className="w-8 h-8 rounded-lg bg-bear/10 flex items-center justify-center">
            ...
            <div>
              ...
              <div
                className={`text-xl font-bold font-mono ${pnlColor(s.totalPnl)}`}
              >
                {fmt(s.totalPnl)}
              </div>
            </div>
      ...
      {s.aiTrades > 0 && (
        <div className="p-2.5 rounded-lg bg-terminal-bg/80 border border-terminal-border">
          ...
          <div className="flex items-center gap-3">
            <div
              className={`text-lg font-bold font-mono ${s.aiWinRate >= 50 ? "text-bull" : "text-bear"}`}
            >
              {fmtPct(s.aiWinRate)}
            </div>
      ...
      {/* Exit Breakdown */}
      {stats.exitBreakdown &&
        Object.values(stats.exitBreakdown).some((v) => v > 0) && (
          <div className="p-2.5 rounded-lg bg-terminal-bg/80 border border-terminal-border">
            <div className="flex items-center gap-1.5 mb-2">
